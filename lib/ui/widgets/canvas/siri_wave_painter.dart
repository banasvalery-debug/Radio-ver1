...
class CustomSiriWave extends StatefulWidget{
  ...
  @override
  _CustomSiriWaveState createState() => _CustomSiriWaveState();
}

class _CustomSiriWaveState extends State<CustomSiriWave> {
  @override
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: DecoratedBox(
        ...
        child: CustomIOS9SiriWave(controller: widget._controller),
      ),
    );
  }
}

class CustomIOS9SiriWave extends StatefulWidget {
  const CustomIOS9SiriWave({
    Key? key,
  ...
  @override
  _CustomIOS9SiriWaveState createState() => _CustomIOS9SiriWaveState();
}

class _CustomIOS9SiriWaveState extends State<CustomIOS9SiriWave>
  ...
  @override
  void didUpdateWidget(covariant CustomIOS9SiriWave oldWidget) {
  ...
  @override
  Widget build(BuildContext context) {
    const supportLinePainter = CustomSupportLinePainter();
    final wavePainter = CustomIOS9SiriWavePainter(
...
class CustomSupportLinePainter extends CustomPainter {
  const CustomSupportLinePainter();

  @override
  void paint(Canvas canvas, Size size) {
    final rect = Rect.fromLTWH(0, size.height / 2, size.width, 1);
    final shader = LinearGradient(
      colors: AppNavigation.theme.isDark ? [_black, _white, _white, _black] : [ _white,_black, _black, _white],
      stops: const [0, .1, .8, 1],
    ).createShader(rect);
  ...
  Color get _white => const Color(0xFFFFFFFF).withOpacity(.7);
  Color get _black => const Color(0xFF111111).withOpacity(.7);

  @override
  bool shouldRepaint(CustomSupportLinePainter oldDelegate) => false;
}
...
class CustomIOS9SiriWavePainter extends CustomPainter {
  CustomIOS9SiriWavePainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final entry in _waves.entries) {
      ...
      for (final sign in [1, -1]) {
        ...
        path.close();
        final paint = Paint()
          ..blendMode = AppNavigation.theme.isDark ? BlendMode.plus : BlendMode.hardLight
          ..color = wave.color;
  ...
  @override
  bool shouldRepaint(CustomIOS9SiriWavePainter oldDelegate) =>
